...
import { BodyType } from './clothing';
import { Setting } from './settings';
import { Style } from './styles';
import { Enhancement } from './styles';

export const nsfwAttributePool = {
  Subject: [
...
export const randomizeNSFW = (): Record<string, any> => {
  ...
  const subject = nsfwAttributePool.Subject[Math.floor(Math.random() * nsfwAttributePool.Subject.length)];
  const action = nsfwAttributePool.Action[Math.floor(Math.random() * nsfwAttributePool.Action.length)];
  const attire = nsfwAttributePool.Attire[Math.floor(Math.random() * nsfwAttributePool.Attire.length)];
  const expression = nsfwAttributePool.Expression[Math.floor(Math.random() * nsfwAttributePool.Expression.length)];
  const setting = nsfwAttributePool.Setting[Math.floor(Math.random() * nsfwAttributePool.Setting.length)];
  const enhancements = nsfwAttributePool.Enhancements[Math.floor(Math.random() * nsfwAttributePool.Enhancements.length)];
  const bodyType = nsfwAttributePool.BodyType[Math.floor(Math.random() * nsfwAttributePool.BodyType.length)]; // Pick random bodyType
  ...
  const attributes = {
    ...
    Style: nsfwAttributePool.Style.find(s => realisticStylePool.includes(s.name)), // Optionally add a random realistic style
  };
